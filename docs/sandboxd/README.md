...
| [PRD-snapshot-fork.md](../PRD-snapshot-fork.md) | Proposed | "Fork" a snapshot: checkpoint a session to a pinned base, then spawn N independent sessions that start from that identical RAM+FS state and diverge (RL parallel rollouts / branch-from-known-state). Runtime primitive already exists (`/restore` is a pure read into a new sandbox); needs base-snapshot + fork op + GC pinning. |
