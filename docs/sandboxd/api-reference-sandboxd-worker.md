...
| `SANDBOXD_POD_MEM_LIMIT` | `""` | Worker pod memory limit in bytes, injected by the operator via the downward API **only when the SandboxTemplate sets `resources.limits.memory`**. When set, the worker caps each sandbox at `this − reserve` so a runaway guest OOM‑kills in its own cgroup instead of the sandboxd agent (agent OOM‑protection). **microVM also uses it to size the guest RAM** (`this − reserve`). Unset ⇒ no cap (today's behavior). See admin‑guide‑crds "Agent OOM‑protection". |
| `SANDBOXD_POD_CPU_LIMIT` | `""` | Worker pod CPU limit in **whole cores** (downward API `limits.cpu`, divisor `1` ⇒ `ceil`), injected by the operator **only on microVM pools that set `resources.limits.cpu`**. The microVM worker boots the guest with `ceil(this)` vCPUs (min 1). Not injected for gVisor pools (keeps their pod template unchanged). Unset ⇒ default 1 vCPU. |
...
| `SANDBOXD_CH_VCPUS` / `SANDBOXD_CH_MEMORY_MIB` | `0` (unset) | microVM only: explicit guest vCPU / memory‑MiB override. Takes precedence over the pod‑limit‑derived size (and over the `SANDBOXD_KATA_CONFIG` defaults). Escape hatch for a hand‑tuned guest; normally leave unset and let `resources.limits` drive the size. |
