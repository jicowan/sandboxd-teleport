...
| `--resume-deadline-seconds` | `SANDBOXD_RESUME_DEADLINE_SECONDS` | `90` | Resume/warm‑up deadline. Must exceed your image's cold start. A resume whose cold‑start (image pull + boot) outruns this used to leave the KV entry wedged in `Resuming`; a **stuck‑`Resuming` self‑heal** now reconciles such an entry against the worker's real `/status` (adopts it to `Running` if the sandbox actually came up, or rolls a failed restore back to `Suspended` for retry). |
| `--worker-reclaim-grace-seconds` | `SANDBOXD_WORKER_RECLAIM_GRACE_SECONDS` | `300` | How long a busy worker binding must stay orphaned before the reclaim sweep returns it to idle; `0` disables reclaim. **Must exceed `--resume-deadline-seconds`** (a slow-but-legitimate resume must not be reclaimed out from under itself) — if you raise the resume deadline for a large image, raise this above it too. |
| `--sweep-interval-seconds` | `SANDBOXD_SWEEP_INTERVAL_SECONDS` | `30` | Idle‑suspend / periodic‑checkpoint / stuck‑`Resuming`‑heal sweep period. Sweeps are O(due) (indexed), so this is a granularity knob, not a scan‑cost one; the checkpoint sweep runs a half‑interval offset to avoid lockstep. |
